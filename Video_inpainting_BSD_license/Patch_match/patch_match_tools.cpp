...
template <class T>
//This function do full search (Brute-Force) search
//input: dispField, inputVol, outputVol, occVol, modVol, patchmatch Params.
//output: new dispField with the match
        ...
        nTupleVolume<T> *occVol, nTupleVolume<T> *modVol, const parameterStruct *params)
{
    ...
    for (k=hPatchSizeT; k< ((dispField->tSize) -hPatchSizeT); k++) //run t_dimension (inner boundary)
        for (j=hPatchSizeY; j< ((dispField->ySize) -hPatchSizeY); j++) //run y_dimension (inner boundary)
        {
            for (i=hPatchSizeX; i< ((dispField->xSize) -hPatchSizeX); i++) //run x_dimension (inner boundary)
            {
                minSSD = FLT_MAX;                                          //set minSSD to FLT_MAX in the begining.
                if (modVol->xSize >0)                                      //if exist modVol
...
template <class T>
//This function run Initialise displacement Field step in PatchMatch3D. It will use old field to initialise if it exist or use random value vice-versa.
//Input:
//departVolume, arrivalVolume: use to calculate dispField from departVol to arrivalVol.
//FirstGuessVolume: if it exist, than use it for initialise, otherwise, assign random value.
//params: PatchMatch Params.
            ...
            nTupleVolume<T> *arrivalVolume, nTupleVolume<T> *firstGuessVolume, nTupleVolume<T> *occVol, const parameterStruct *params)
{
    ...
    //run for all point in dispdField (3-D point)
    for (i=0; i< (dispField->xSize); i++)   //for x-dimension
        for (j=0; j< (dispField->ySize); j++) //for y-dimension
            for (k=0; k< (dispField->tSize); k++) //for t-dimension
			{
                isNotOcc = 0;               //first assume that guess is occlude, we will check later
                ...
                while(isNotOcc == 0)
                {
                        ...
                        )
                    {
                            ...
                            k+(int)firstGuessVolume->get_value(i,j,k,2)) )
                        {
                            ...
                            continue;
                                ...
                                //BUild a range for random search around each point, region control by variable w
                        {
                    ...
                    if (arrivalVolume->xSize == arrivalVolume->patchSizeX)	//special case where the patch size is the size of the dimension
                    {
                        ...
                        if ( (dispField->get_value(i,j,k,0) == FLT_MAX) || (firstGuessVolume->xSize == 0))  //default behaviour, no init or not in the inner boundary
                        {
                            xDisp = ((rand()%( (arrivalVolume->xSize) -2*hPatchSizeCeilX-1)) + hPatchSizeX)-i; //random search
                        }
                        else    //based on an initial guess
                        {
                            xDisp = (int)(round_float(rand_float_range((float)(xMin),(float)(xMax))) - i); //random around init point
                        }
                    }
                ...
                dispField->set_value(i,j,k,0, (T)(xDisp)); //x_displacement
                dispField->set_value(i,j,k,1, (T)(yDisp)); //y_displacement
                dispField->set_value(i,j,k,2, (T)(tDisp)); //t_displacement

                //check departVolume if it is in inner boundary then calculate the ssd Error.
...
template <class T>
//this function do random search around best match to prevent convergence to local minima.
//perform in different search indice.
        ...
        nTupleVolume<T> *occVol, nTupleVolume<T> *modVol, const parameterStruct *params)
{
    ...
    int xRand,yRand,tRand; //init seed
    int randMinX,randMaxX,randMinY,randMaxY,randMinT,randMaxT; //range of random search
    ...
    nTupleVolume<int> *wValues = new nTupleVolume<int>(1,zMax,1,1,imgVolA->indexing); //array store z params
    //store the values of the maximum search parameters
    for (int z=0; z<zMax; z++) //search range
    {
...
template <class T>
//This function do Patch_match propagation (propagate good match follow bottom and right) with assumption that image are strongly coherent.
//input: dissFiend(init field), depart and arrival Volume, occlussion Volume, modVolume, patchmatchParams, number of iteration.
//output: Number of modified in dispField, new dispField.
        ...
        nTupleVolume<T> *modVol, const parameterStruct *params, int iterationNb)
{
    ...
    nbModified = 0;     //number of modifier is zeros
        ...
        for (k=((dispField->tSize)-1); k>= 0; k--)
			for (j=((dispField->ySize) -1); j>= 0; j--)
                for (i=((dispField->xSize) -1); i>= 0; i--)
				{
                    ...
                    currentError = dispField->get_value(i,j,k,3); //get current error.
					...
					{
                        dispField->set_value(i,j,k,3,currentError); //SSD error of dispField
					}
