...
#include "reconstruct_video_and_features.h"
        ...
        nTupleVolume<T>* dispField, float sigmaColour, int useAllPatches, int reconstructionType)
{
    ...
    int nbNeighbours;
    int correctInfo;
    float alpha, adaptiveSigma,gammaWeight;
    float *weights,sumWeights, avgColourR, avgColourG, avgColourB, *colours;
    float avgGradX,avgGradY,avgNormGradX,avgNormGradY;
        ...
        for (j=0; j<(occVol->ySize); j++)
            ...
            {    
                ...
                else    /*an occluded pixel (therefore to be modified)*/
                {
                        ...
                        for (jj=jMin; jj<=jMax;jj++)
                            for (ii=iMin; ii<=iMax;ii++)
                            {
                                if (useAllPatches)
                                {
                                    ...
                                    avgGradX = avgGradX + (float)(weights[weightInd])*(gradXvol->get_value(xDispShift,yDispShift,tDispShift,0));
                                    avgGradY = avgGradY + (float)(weights[weightInd])*(gradYvol->get_value(xDispShift,yDispShift,tDispShift,0));
                    ...
                    gradXvol->set_value(i,j,k,0,(T)(avgGradX/(sumWeights)));
                    gradYvol->set_value(i,j,k,0,(T)(avgGradY/(sumWeights)));
                    normGradXvol->set_value(i,j,k,0,(T)(avgNormGradX/(sumWeights)));
                    normGradYvol->set_value(i,j,k,0,(T)(avgNormGradY/(sumWeights)));
